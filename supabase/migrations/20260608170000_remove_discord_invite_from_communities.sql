...
select
  c.id,
  'DISCORD',
  trim(c.discord_invite),
  coalesce(positions.next_position, 0),
  0,
  'DISCORD'
from public.communities c
left join lateral (
  select max(cl.position) + 1 as next_position
  from public.community_links cl
  where cl.community_id = c.id
) positions on true
where nullif(trim(c.discord_invite), '') is not null
  and not exists (
    select 1
    from public.community_links existing_link
    where existing_link.community_id = c.id
      and existing_link.platform = 'DISCORD'
      and existing_link.url = trim(c.discord_invite)
  );

alter table public.communities
drop column if exists discord_invite;
